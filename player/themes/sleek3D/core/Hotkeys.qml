...
Rectangle {
	...
	function keys(event) {
		...
		if (core.getComponent('contextMenu').get('visible') === true){
			core.getComponent('contextMenu').close();
		}
			...
			if (event.key == Qt.Key_Right) {
				...
				if (typeof settings.preventKey[Qt.ControlModifier+"+"+Qt.Key_H] === "undefined") {
					...
					if (settings.uiVisible) {
						app.infoMsg("UI Visible");
					} else app.infoMsg("UI Hidden");
					settings = settings;
					return;
				}
			}
		...
		if(event.modifiers == Qt.ShiftModifier) {
			if (event.key == Qt.Key_Right) {
				if (typeof settings.preventKey[Qt.ShiftModifier+"+"+Qt.Key_Right] === "undefined") {
					if(settings.subtitles3DOffset < 20){
						settings.subtitles3DOffset += 1;
					}
				...
				if (typeof settings.preventKey[Qt.ShiftModifier+"+"+Qt.Key_Left] === "undefined") {
					if(settings.subtitles3DOffset > 0){
						settings.subtitles3DOffset -= 1;
					}
					return;
				}
		...
		if (event.key == Qt.Key_M) {
			if (typeof settings.preventKey[Qt.Key_M] === "undefined") {
				...
				if (vlcPlayer.audio.mute) {
					...
					app.infoMsg("Volume " + (Math.round((250 * (vlcPlayer.volume /200))/10) *5) + "%");
				}
				wjs.refreshMuteIcon();
			}
		}
		...
		if (event.key == Qt.Key_Z) {
			if (typeof settings.preventKey[Qt.Key_Z] === "undefined") {
				...
				for (kl = 0; typeof settings.zooms[kl] !== 'undefined'; kl++) if (settings.curZoom == kl) {
					...
					app.infoMsg("Zoom Mode: " + settings.zooms[settings.curZoom][1]);
					break;
				}
			}
		}
		...
		if (event.key == Qt.Key_G) {
			if (typeof settings.preventKey[Qt.Key_G] === "undefined") {
				VPlayer3D.Subtitles.decreaseDelay();

			}
		}
		if (event.key == Qt.Key_H) {
			if (typeof settings.preventKey[Qt.Key_H] === "undefined") {
				VPlayer3D.Subtitles.increaseDelay();
			}
		}
		if (event.key == Qt.Key_Up) {
			if (typeof settings.preventKey[Qt.Key_Up] === "undefined") {
				wjs.volumeTo(8);
				return;
			}
		} else if (event.key == Qt.Key_Down) {
			if (typeof settings.preventKey[Qt.Key_Down] === "undefined") {
				wjs.volumeTo(-8);
				return;
			}
		}
/*
		if (event.key == Qt.Key_J) {
			if (typeof settings.preventKey[Qt.Key_J] === "undefined") {
				...
				app.infoMsg("Audio Delay: "+settings.audioDelay+" ms");
...
*/
	...
	function mouseDblClick(clicked) {
		if (settings.multiscreen == 0) {
			if (settings.mouseevents == 1) {
				// JavaScript Mouse Events Demo
				if (vlcPlayer.state != 1) if (core.getComponent('toolbarBackground').getContainsMouse() === false) if (clicked == Qt.LeftButton) {
					var sendjsdata = {};
	...
	function mouseClick(clicked) {
		try{
			...
			if (core.getComponent('toolbarBackground').getContainsMouse() === false) {
				var sendjsdata = {};
				if (clicked == Qt.RightButton) {
					...
					if (settings.cursorX == 0 && settings.cursorY == 0) { } else {
						if (settings.multiscreen == 1 && fullscreen === false) { } else {
							core.getComponent('contextMenu').open();
						}
					}
				} else if (clicked == Qt.MiddleButton){
					buttons.clicked('fullscreen');
				} else {
					if (core.getComponent('contextMenu').get('visible') === true) {
						core.getComponent('contextMenu').close();
					} else {
